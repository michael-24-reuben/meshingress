...
This entry is resolved for the repository MVP and runtime publication acceptance gate. Remaining unchecked scanner, storage hardening, UI, repository-client, SBOM, and external security-tool tasks are deferred follow-up scope and should be promoted into focused future architect entries before implementation.
...
- [ ] Add publication client to `meshingress-server`. Deferred; MVP installs a signed publication JSON payload directly.
...
- [ ] Verify executable checksums before execution. Deferred for non-JAR or extracted-executable install flows; current MVP verifies JAR checksum before and after runtime-cache copy.
...
- [x] Runtime test rejects unsigned publication.
- [x] Runtime test rejects checksum mismatch.
- [x] Runtime test rejects revoked artifact.
