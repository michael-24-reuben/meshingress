...
- Optional: `title`, `description`, `version`, `enabled`, `visibility`, `invocationName`, `defaultFunction`, `handlerKey`, `dynamic`
- Tool ID pattern: `[a-z][a-z0-9]*(\.[a-z0-9]+)*`
- Invocation name defaults to the tool ID if not set
...
This example shows a complete annotation-based tool with:

- class-level tool metadata and scopes
- two functions with their own scopes and configuration
- record-based inputs with `@McpInputField`
- method-parameter inputs with `@McpFunctionParam`
- secrets and availability configuration
