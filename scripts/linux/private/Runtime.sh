#!/usr/bin/env bash
#
# Starts and manages the Meshingress application, including its MCP and artifact surfaces.
#
# The packaged Spring Boot JAR runs as a managed background process with separate
# stdout/stderr logs under var/logs/meshingress. The JSON state file under var/run
# records only processes started by this launcher.
#
...
Actions:
  ...
  Stop       Stop the application owned by this launcher
  Status     Show managed application status
  Logs       Follow managed application logs
...
Options:
  ...
  --build                          Build the application JAR before starting
...
find_project_root() {
    ...
    while [[ -n "$directory" && "$directory" != "/" ]]; do
        if [[ -f "$directory/pom.xml" && -d "$directory/app/meshingress-server" ]]; then
            printf '%s\n' "$directory"
            return 0
        fi
    ...
    if [[ -f "/pom.xml" && -d "/app/meshingress-server" ]]; then
        printf '/\n'
        return 0
    fi
...
ASCII_PATH="$PROJECT_ROOT/data/assets/logo/meshingress-bloody.ascii.txt"
...
# Version 1 state files had a separate repository process. Retain that entry
# only so Status, Logs, and Stop can safely clean up an older launcher run.
...
state_has_service() {
    local name="$1"
    [[ -n "${STATE_PID[$name]-}" || -n "${STATE_JAR[$name]-}" ]]
}
...
stop_managed_services() {
    ...
    for name in server repository; do
        state_has_service "$name" || continue
        pid="${STATE_PID[$name]-}"
        jar="${STATE_JAR[$name]-}"
...
invoke_build() {
    ...
    local -a arguments=(-pl "app/meshingress-server" -am)
...
write_state() {
    ...
    python3 - \
        ...
        "${SERVICE_PID[server]}" "${SERVICE_JAR[server]}" "${SERVICE_STDOUT[server]}" "${SERVICE_STDERR[server]}" "${SERVICE_STARTED_AT[server]}" <<'PY'
...
show_meshingress_control_center() {
    ...
    while true; do
        ...
        write_meshingress_panel "MESHINGRESS CONTROL CENTER" 93 \
            ...
            "Artifacts     http://$(get_probe_address "$SERVER_ADDRESS"):$SERVER_PORT/artifact" \
            "Started       $started_at" \
            "Uptime        $uptime" \
...
write_host_logged "Starting Meshingress application - [$(date '+%Y-%m-%d %H:%M:%S')]" "INFO" "launcher" "LAUNCH"
...
if [[ "$BUILD" == true || ! -f "$SERVER_JAR" ]]; then
    invoke_build
fi
[[ -f "$SERVER_JAR" ]] || die "Packaged Meshingress application jar is missing after the build: $SERVER_JAR"
...
write_host_logged "Meshingress application is running." "INFO" "service" "RUNNING"
