...
export function WorkflowCanvas({nodes, edges, selectedNodeId, nodeRunStates, onSelect, onNodesChange}: WorkflowCanvasProps) {
    ...
    const updatePosition = (id: string, rawX: number, rawY: number) => onNodesChange((current) => {
        ...
        const x = snapToGrid(clamp(rawX, NODE_STAGE_INSET, worldSize.width - node.width - NODE_STAGE_INSET))
        const y = snapToGrid(clamp(rawY, NODE_STAGE_INSET, worldSize.height - NODE_HEIGHT - NODE_STAGE_INSET))
    ...
    const onPointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
        if (pan.current?.pointerId === event.pointerId) setTransform((current) => ({
            ...current,
    ...
    return <div aria-label="Workflow canvas. Drag empty space to pan and use the mouse wheel to zoom." className={`canvas${pan.current ? ' panning' : ''}`} onPointerCancel={endPan} onPointerDown={onPointerDown}
                onPointerMove={onPointerMove} onPointerUp={endPan} onWheel={onWheel} ref={canvasRef} tabIndex={0}
        ...
        <div className="canvas-stage" style={{width: worldSize.width, height: worldSize.height, transform: `translate(${transform.x}px, ${transform.y}px) scale(${transform.zoom})`}}>
            <svg aria-hidden="true" id="edge-layer" style={{width: worldSize.width, height: worldSize.height}}>{routes.map(({edge, path}) => <path className={`edge${edge.target === selectedNodeId ? ' active' : ''}`} d={path} key={`${edge.source}-${edge.target}`}/>)}</svg>
            {nodes.map((node) => <WorkflowNode key={node.id} node={node} runState={nodeRunStates[node.id] ?? 'idle'} selected={node.id === selectedNodeId} zoom={transform.zoom} onDrag={updatePosition}
                                               onSelect={onSelect}/>)}</div>
    </div>
}

function WorkflowNode({node, selected, runState, zoom, onSelect, onDrag}: {
    node: WorkflowNode;
    selected: boolean;
    ...
    return <article className={`node${selected ? ' selected' : ''}${runState === 'idle' ? '' : ` ${runState}`}`} data-node-id={node.id} onClick={() => onSelect(node.id)}
                    style={{left: node.x, top: node.y, width: node.width}}>
        <div className="node-head" onPointerCancel={endDrag} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={endDrag}>
            ...
            <NodeIcon nodeKind={node.kind} toolId={node.toolId}/>
            </span>
