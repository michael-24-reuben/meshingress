...
import type { LogEntry, RegisteredTool, StudioView, WorkflowNode } from '../types'

const files = [{ name: 'desktop-volume-greeting-solo-leveling.json', path: 'samples', size: '1.6 KB', group: 'Workflow samples' }]

export function ExplorerPanel({ view, nodes, logs, tools, toolsState, onSelectNode, onStatus }: { view: StudioView; nodes: WorkflowNode[]; logs: LogEntry[]; tools: RegisteredTool[]; toolsState: 'loading' | 'ready' | 'error'; onSelectNode: (id: string) => void; onStatus: (message: string) => void }) {
  ...
  const { title, placeholder, body } = useMemo(() => {
    ...
    if (view === 'tools') {
      const toolList = tools.filter((tool) => matches(`${tool.id} ${tool.functions.map((functionEntry) => functionEntry.name).join(' ')}`))
      const body = toolsState === 'loading'
        ? <Empty message="Loading registered tools…" />
        : toolsState === 'error'
          ? <Empty message="Unable to load registered tools." />
          : toolList.length
            ? <div className="list">{toolList.map((tool) => <ListItem icon="T" key={tool.id} meta={`${tool.functions.length} fn`} subtitle={tool.functions.map((functionEntry) => functionEntry.name).join(', ')} title={tool.id} />)}</div>
            : <Empty message="No registered tools." />
      return { title: 'Registered tools', placeholder: 'Filter registered tools', body }
    }
