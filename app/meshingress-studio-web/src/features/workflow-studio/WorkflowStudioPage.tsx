...
import type { DrawerView, InspectorView, LogEntry, NodeRunState, RegisteredTool, StudioView, WorkflowRunResult } from './types'
import './workflow-studio.css'

const sampleRunPath = '/api/v1/workflows/samples/desktop-volume-greeting-solo-leveling/run'

function groupRegisteredTools(functions: McpToolFunction[]): RegisteredTool[] {
  const tools = new Map<string, RegisteredTool>()
  functions.forEach((functionEntry) => {
    const separator = functionEntry.name.lastIndexOf('.')
    if (separator < 1 || separator === functionEntry.name.length - 1) return
    const toolId = functionEntry.name.slice(0, separator)
    const functionName = functionEntry.name.slice(separator + 1)
    const tool = tools.get(toolId) ?? { id: toolId, functions: [] }
    tool.functions.push({ name: functionName, description: functionEntry.description })
    tools.set(toolId, tool)
  })
  return [...tools.values()]
    .map((tool) => ({ ...tool, functions: tool.functions.toSorted((left, right) => left.name.localeCompare(right.name)) }))
...
export function WorkflowStudioPage() {
  ...
  useEffect(() => {
    ...
    void listMcpTools()
      .then((functions) => {
        if (cancelled) return
        setTools(groupRegisteredTools(functions))
        setToolsState('ready')
      })
      .catch(() => {
