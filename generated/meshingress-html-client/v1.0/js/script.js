...
function renderSelectedTool() {
    ...
    if (!tool) {
        ...
        elements.toolDetails.classList.add("empty-state");
...
function updateSelectedToolDetailsOnly() {
    ...
    elements.toolDetails.classList.remove("empty-state");
...
function renderToolNavigation() {
    evaluateToolPopulation(state.tools);
...
function evaluateToolPopulation(tools) {
    if (tools.length === 0) {
        elements.toolNavigationMount.classList.add("empty-state");
        replaceChildren(elements.toolNavigationMount, [
            createElement("div", { className: "empty-state", text: "No tools available." })
        ]);
    } else {
        elements.toolNavigationMount.classList.remove("empty-state");
    }
}
